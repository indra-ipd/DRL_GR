...
class GridGraph(object):
    ...
    def reset(self):
        ...
        if self.twopin_pt >= len(self.twopin_combo):
            ...
            self.net_ind = 0
            self.pair_ind = 0
            errVal = np.mean(np.asarray(self.training_error))
            elapse = time.time()-self.startTime
            ...
            if sum([1 for item in self.instantrewardcombo if item > 0]) > self.posTwoPinNum:
                ...
                if self.reward>self.best_reward:
                    ...
                    self.best_route = self.route_combo
                    is_best = 1
            resStr = 'Episode {ep} Reward {rd} Pins {p}/{pt} Loss {err} Time {time}\n'.format(ep=self.episode,rd=self.reward,p=sum([1 for item in self.instantrewardcombo if item > 0]),pt=len(self.twopin_combo), err=errVal, time=elapse)
            with open('updateFreq.txt','a') as myFile:
                myFile.write('Episode {ep}\n'.format(ep=self.episode))
            with open('myResults.txt','a') as myFile:
                ...
                '''if self.episode == 500:
                    myFile.write('\n********************\n')
                    myFile.write(str(self.training_error))
                    myFile.write('\n********************\n')'''
            print('Positive two pin num: {}/{}'.format(sum([1 for item in self.instantrewardcombo if item > 0]),len(self.twopin_combo)))
            pinsRouted = sum([1 for item in self.instantrewardcombo if item > 0])
        ...
        return self.current_state, reward_plot, is_best,pinsRouted
